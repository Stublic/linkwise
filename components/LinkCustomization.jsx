...
const LinkCustomization = ({
  ...
  // Declare linkDataFromDatabase in the component's scope
  const [linkDataFromDatabase, setLinkDataFromDatabase] = useState([]);
  ...
  useEffect(() => {
    ...
    prisma.link.findMany().then((dbLinks) => {
      if (dbLinks.length > 0) {
        ...
        const linkData = dbLinks.map((link) => ({
        ...
        setLinkDataFromDatabase(linkData);
      }
    });
  }, []);
